...
const SalesManagement = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <Title order={1} mb="lg" className="text-blue-700 font-bold">Gestión de Ventas</Title>
      ...
      <Modal opened={opened} onClose={close} title="Registrar Nueva Venta" size="lg">
        <form onSubmit={handleRegisterSale}>
          ...
          {items.map((item, index) => (
            <Paper key={index} p="sm" withBorder mb="sm" className="rounded-lg border-gray-200">
              ...
              <Group position="apart" mt={4}>
                <Text size="xs" color="dimmed">Stock disponible: {item.stock}</Text>
                <Text size="xs" color="blue">Subtotal: S/ {(item.quantity * item.salePrice).toFixed(2)}</Text>
              </Group>
            </Paper>
          ))}
          ...
          <Paper p="md" mt="md" withBorder className="rounded-lg bg-gray-50">
            <Group position="apart">
          ...
          <Group position="right" mt="xl">
            <Button type="submit" color="blue">Guardar Venta y Actualizar Stock</Button>
          </Group>
        </form>
      </Modal>

      <Button onClick={open} color="blue" radius="md" className="mb-4">Registrar Nueva Venta</Button>
